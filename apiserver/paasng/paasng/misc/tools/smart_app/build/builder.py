...
class SmartAppBuilder:
    ...
    def __init__(
        self,
        smart_build_record: "SmartBuildRecord",
        source_get_url: str,
        dest_put_url: str,
        artifact_download_url: str,
    ):
        self.smart_build_record = smart_build_record
        self.source_get_url = source_get_url
        self.dest_put_url = dest_put_url
        self.artifact_download_url = artifact_download_url

        self.stream = make_channel_stream(smart_build_record)
        self.state_mgr = SmartBuildStateMgr.from_smart_build_id(
            smart_build_id=smart_build_record.uuid, phase_type=SmartBuildPhaseType.PREPARATION
        )
        self.procedure = partial(SmartBuildProcedure, self.stream, self.smart_build_record)

    def start(self):
        """Start the s-mart building process"""

        # 开始准备阶段
        start_phase(self.smart_build_record, self.stream, SmartBuildPhaseType.PREPARATION)
        preparation_phase = self.smart_build_record.phases.get(type=SmartBuildPhaseType.PREPARATION)

        with self.procedure("校验应用描述文件", phase=preparation_phase):
            self.validate_app_description()

        # 结束准备阶段
        end_phase(self.smart_build_record, self.stream, JobStatus.SUCCESSFUL, SmartBuildPhaseType.PREPARATION)

        # 开始构建阶段
        start_phase(self.smart_build_record, self.stream, SmartBuildPhaseType.BUILD)
        build_phase = self.smart_build_record.phases.get(type=SmartBuildPhaseType.BUILD)

        self._start_build_process(build_phase)

        # 结束构建阶段
        end_phase(self.smart_build_record, self.stream, JobStatus.SUCCESSFUL, SmartBuildPhaseType.BUILD)

        self._finial_builder()
    ...
    def _start_build_process(self, build_phase: "SmartBuildPhase"):
        ...
        with self.procedure("构建 S-Mart 包", phase=build_phase):
            ...
            params = {
                "smart_build_id": self.smart_build_record.uuid,
    ...
    def _finial_builder(self):
        """结束整体的构建流程"""

        self.stream.write_event("EOF", {"build_id": str(self.smart_build_record.uuid)})
        self.stream.close()

        self.state_mgr.coordinator.release_lock(self.smart_build_record)
    ...
    def start_following_logs(self, builder_name: str):
        ...
        self.smart_build_record.update_fields(status=JobStatus.SUCCESSFUL)
    ...
    def launch_build_process(self):
        ...
        cluster_name = get_default_cluster_name()
        namespace = get_default_builder_namespace()
        pod_name = generate_builder_name(self.smart_build_record)
...
def start_phase(smart_build_record: "SmartBuildRecord", stream: "SmartBuildStream", phase: SmartBuildPhaseType):
    """开始阶段"""
    phase_obj: "SmartBuildPhase" = smart_build_record.phases.get(type=phase.value)
    phase_obj.mark_and_write_to_stream(stream, JobStatus.PENDING)


def end_phase(
    smart_build_record: "SmartBuildRecord", stream: "SmartBuildStream", status: JobStatus, phase: SmartBuildPhaseType
):
    """结束阶段"""
    phase_obj: "SmartBuildPhase" = smart_build_record.phases.get(type=phase.value)
