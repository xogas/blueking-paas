...
class SmartBuildStateMgr:
    ...
    def __init__(
        ...
        stream: Optional[SmartBuildStream] = None,
    ):
        self.smart_build_record = smart_build_record
        self.stream = stream or get_default_stream(smart_build_record)
        self.phase_type = phase_type
        self.coordinator = SmartBuildCoordinator(f"{smart_build_record.operator}:{smart_build_record.app_code}")

    @classmethod
    def from_smart_build_id(cls, smart_build_id: str, phase_type: "SmartBuildPhaseType"):
        smart_build_record = SmartBuildRecord.objects.get(pk=smart_build_id)
        return cls(smart_build_record, phase_type)

    def update(self, **fields):
        return self.smart_build_record.update_fields(**fields)
