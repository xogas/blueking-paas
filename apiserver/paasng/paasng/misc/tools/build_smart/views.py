...
class SmartBuilderViewSet(viewsets.ViewSet):
    ...
    def build_smart(self, request):
        """根据暂存的源码包构建 s-mart 包"""
        slz = SmartPackageBuildInputSLZ(data=request.data)
        slz.is_valid(raise_exception=True)
        ...
        with generate_temp_dir() as tmp_dir:
            ...
            # 3. 保证同一时间构建的唯一性
            coordinator = SmartBuildCoordinator(stat.sha256_signature)
        ...
        smart_build: Optional[SmartBuildRecord] = None
        try:
            ...
            self._handle_smart_build_failed(smart_build, e)

        build_id = smart_build.id if smart_build else ""
        return JsonResponse(
            data={"smart_build_id": build_id, "stream_url": f"/streams/{build_id}"},
    ...
    def _handle_smart_build_failed(self, smart_build: Optional[SmartBuildRecord], error: Exception):
        # TODO: 处理构建失败的逻辑
        logger.error("Smart build failed", exc_info=error)
